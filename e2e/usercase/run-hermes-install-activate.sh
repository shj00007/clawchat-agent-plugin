...
# ── 3. 发 prompt：安装 + 激活 ──────────────────────────────────────────────
PROMPT="Strictly follow the instruction from https://plugin.clawling.chat/clawchat/install-dev.md to install and activate clawchat plugin. The active code is ${CODE}."
AGENT_OUT="$(mktemp)"
log "③ 发 prompt 驱动 agent 安装+激活（剩余 $(remaining)s）"
# hermes chat 一次性 agent turn：模型按 install-dev.md 跑 npx 安装 + `hermes clawchat activate <code>`。
( timeout "$(remaining)" kc exec "$POD" -- bash -lc \
    "source /opt/hermes/.venv/bin/activate 2>/dev/null; hermes chat -q $(printf '%q' "$PROMPT")" \
    >"$AGENT_OUT" 2>&1 ) &
AGENT_PID=$!

# ── 4. 拉起 gateway 并轮询「连接上」───────────────────────────────────────
# 激活会持久化凭据并调度一次 detached `hermes gateway restart`；这里再显式起一个
# 受控日志的 gateway（restart 幂等），确保 WS 真正建立且日志可被 grep。
gateway_started=0
SUCCESS=0
log "④ 等待激活落库 → 拉起 gateway → 轮询 WebSocket 握手（硬超时 ${TIMEOUT_SECONDS}s）"
while (( $(remaining) > 0 )); do
  # 激活凭据是否已落库/落盘？
  if (( gateway_started == 0 )) && kc exec "$POD" -- bash -lc \
       'test -s /opt/data/.env && grep -q CLAWCHAT_TOKEN /opt/data/.env' 2>/dev/null; then
    log "   激活凭据已写入 /opt/data/.env，启动 gateway"
    kc exec "$POD" -- bash -lc \
  ...
  # 镜像内无 sqlite3 CLI，用容器自带 python3 查（stdin heredoc 免去引号转义）。
